...
//======================================================================
  ...
  while(1)
  {
    inviaMappa(sd,Mappa2);
    read(sd,&spostamento,1);

    if(spostamento != '@' && (spostamento == 'a' || spostamento == 's' || spostamento == 'd' || spostamento == 'w'))
    {
     printf("\nRicevuto da Client_socket=%d spostamento a [%c]\n",sd,spostamento);
     movementCharacterClient(Mappa2,spostamento,&Coordinata_X,&Coordinata_Y);
    }
  }
